...
The package name was available during planning; that check does not reserve it.
...
`benchmark-evidence.json`. Prepare a release locally from the exact verified files:

```sh
node scripts/release.ts prepare --benchmark-dir artifacts/benchmark/run-EXACT
