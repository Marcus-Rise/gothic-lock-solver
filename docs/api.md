...
The runtime exports are `solveLock`, `LockInputError` and `SearchLimitError`.
...
import type { State, Links, Command, Position, Delta, Link, SearchLimit } from "gothic-lock-solver";
```
...
`solveLock(state, links)` takes no third options object. `state[i]` is the numeric
...
`LockInputError` signals malformed input. `SearchLimitError` exposes a documented
...
```sh
node dist/gothic-lock-solver.cli.mjs examples/lock.example.json
node dist/gothic-lock-solver.cli.mjs examples/lock.example.json --output solution.json
# Equivalent installed npm bin:
gothic-lock-solver lock.json --output solution.json
# Repository source entry retained for development:
node solve-lock.mjs examples/lock.example.json
```

The input file is a JSON object with `state` and `links`. The console is Russian
...
Use the paired [benchmark harness](../benchmarks/README.md) for algorithm timings;
