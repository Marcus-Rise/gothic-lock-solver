...
import { loadCatalog } from '../../tests/benchmarks/catalog.ts';
      ...
      try {
        for (const filename of runtimeFiles) {
          ...
          try {
            ...
            }, { moduleUrl: url, moduleFormat: filename.endsWith('.mjs'), definitions: locks.map(({ state, links }) => ({ state, links })) });
            ...
            for (const [index, result] of results.entries()) {
              ...
              if (commands.length !== lock.expectedActions || replayCommands(lock, commands).some(position => position !== 4)) throw new Error(`CDN fixture mismatch: ${engine.name()}/${filename}/${lock.id}`);
            }
