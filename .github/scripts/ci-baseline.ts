...
if (!['checkout', 'release'].includes(mode ?? '') || !directoryArgument || !outputArgument) throw new Error('Usage: node .github/scripts/ci-baseline.ts checkout BASE_CHECKOUT OUTPUT_DIR | release CANDIDATE_VERSION OUTPUT_DIR');
...
if (mode === 'checkout') {
  ...
  execFileSync(process.execPath, ['tests/benchmarks/snapshot.ts', '--source-module', module, '--source-format', format,
  ...
  if (response.status === 404) {
    ...
    if (previous === null) {
      ...
      await writeJson(snapshotPath, { ...snapshot, source: { ...source, releaseVersion: previous, kind: 'previous-stable-release', releaseManifestSha256: sha256(manifestBytes), releaseEvidenceSha256: sha256(evidenceBytes) } });
