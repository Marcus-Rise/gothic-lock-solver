import { LockModel } from './lock-model.ts';
import { MatrixSearch } from './matrix-search.ts';
import { PreparedSearch, SearchBudget } from './search-limits.ts';
 ...
 * @throws {LockInputError} When either input violates the lock model.
...
export function solveLock<const S extends readonly Position[]>(
  ...
  return new MatrixSearch(model, prepared, new SearchBudget()).solve();
}
