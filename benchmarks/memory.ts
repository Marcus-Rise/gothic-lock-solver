...
export type MemorySpec = { name: string; module: string; format: ModuleFormat | 'upstream' };
...
export function measureRss(spec: MemorySpec, fixtureId?: string): RssSample {
  const worker = fileURLToPath(new URL('../scripts/benchmark-memory-worker.ts', import.meta.url));
