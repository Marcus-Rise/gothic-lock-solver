...
for (const filename of ['gothic-lock-solver.mjs', 'gothic-lock-solver.min.mjs']) {
  test(`${filename}: real Node loader solves all 45 fixtures at the exact minimum`, () => {
    ...
    const code = `import { solveLock } from ${JSON.stringify(new URL(`../../dist/${filename}`, import.meta.url).href)};let input='';for await (const chunk of process.stdin) input+=chunk;console.log(JSON.stringify(JSON.parse(input).map(({definition:{state,links}})=>solveLock(state,links))));`;
    ...
    for (const [index, fixture] of catalog.entries()) {
      ...
      expect(replayCommands(fixture.definition, commands)).toEqual(fixture.definition.state.map(() => 4));
    }
  });
}
