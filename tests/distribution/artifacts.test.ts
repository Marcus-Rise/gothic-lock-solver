...
describe('published artifacts', () => {
  ...
  for (const filename of runtime.filter((name) => name.endsWith('.mjs'))) {
    test(`${filename} exposes the actual ESM namespace`, async () => {
      ...
      const output = node.execFileSync(process.execPath, ['--input-type=module', '-e', `import { solveLock } from ${JSON.stringify(url)}; import * as api from ${JSON.stringify(url)}; console.log(JSON.stringify([solveLock([6,2],[[0,-1],[0,0]]),api.solveLock([4,4],[[0,0],[0,0]]),Object.keys(api).sort()]));`], { encoding: 'utf8', cwd: fileURLToPath(dist) });
      expect(JSON.parse(output)).toEqual([[[0, -2]], [], ['LockInputError', 'SearchLimitError', 'solveLock']]);
    });
  }
