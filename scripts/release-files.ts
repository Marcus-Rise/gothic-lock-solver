...
export async function prepareRelease(options: PrepareOptions): Promise<ReleaseManifest> {
  ...
  const notes = [
    ...
    'This release provides the two-argument tuple API: solveLock(state, links). Commands are [zero-based index, signed numeric pin delta]. See README for migration from the legacy result wrapper.', '',
